...
export function Sidebar({ isAdmin, mobileOpen = false, onMobileClose }: SidebarProps) {
    ...
    return (
      <>
        ...
        <div className="flex items-center h-16 px-4 border-b border-border shrink-0">
          ...
          {forMobile && onMobileClose && (
            <button
              onClick={onMobileClose}
              className="ml-auto p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent/50 transition-colors"
            >
              <X size={18} />
            </button>
          )}
          ...
          {isAdmin && (
            <>
              ...
              >
                ...
                {showLabel && <span className="text-sm">Admin Panel</span>}
              </Link>
            </>
          )}
        </nav>
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="relative hidden lg:flex flex-col h-full bg-card border-r border-border overflow-hidden"
      >
        <NavContent />
        <button
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.aside
            ...
            className="fixed left-0 top-0 bottom-0 z-50 w-72 flex flex-col bg-card border-r border-border overflow-hidden shadow-2xl lg:hidden"
          >
